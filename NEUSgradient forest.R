...
#Wind.Speed<-NEUSgfa$d$Wind.Speed$x
NAO_w<-NEUSgfa$d$NAO_w$x
GS<-NEUSgfa$d$GS$x
Income<-NEUSgfa$d$Income$x
Precipitation<-NEUSgfa$d$Precipitation$x

NEUS.dens<-cbind(Population, SST, AMO, Seafood, Exploitation, MEI,  Landings, 
              NAO_w, GS, Income, Precipitation)

NEUS.dens<-as.data.frame(NEUS.dens)

neus.d<-sapply(NEUS.dens, summary)
write.csv(neus.d, file="NEUS.dens.split.csv")
